...
if not inputres:
    ...
    command = '/users/rhiju/python/decide_termini_truncate.py '+secstructprobfile[0]+ ' ' + outfile
    print(command)
    system(command)
...
if(len(infile)>0): # PDB file is optional.
    ...
    command = '/users/rhiju/python/termini_truncate_pdb.py %s %d %d %s' % \
...
infile = infile[0]
outfile = outdir + '/'+newprefix+basename(infile)
command = '/users/rhiju/python/termini_truncate_fasta.py %s %d %d %s' % \
          (infile,startseq,endseq,outfile)
print(command)
system(command)
...
infile = infile[0]
outfile = outdir + '/'+newprefix+basename(infile)
command = '/users/rhiju/python/termini_truncate_psipred_ss2.py %s %d %d %s' % \
          (infile,startseq,endseq,outfile)
print(command)
system(command)
...
for infile in infiles:
    ...
    command = '/users/rhiju/python/termini_truncate_fragfile.py %s %d %d %s' % \
              (infile,startseq,endseq,outfile)
    print(command)
    system(command)
