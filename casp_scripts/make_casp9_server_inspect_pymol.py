...
servers_all = ["HHpredA","HHpred2","BAKER","ROBETTA","Zhang","Midway","RAPTOR_"]
...
for target_num in target_nums:
    ...
    #####################################
    if not ( exists( "rosetta_frags/" ) ):
        print "... Could not find: "+workdir+"/rosetta_frags"
        print "... Trying to download it from ade."
        command = "ssh ade.stanford.edu pick_frags_casp9.py %d" % target_num
        print command
        system( command )

        command =  "rsync_from_cluster.py ade.stanford.edu rosetta_frags"
        print command
        system( command )
        if not ( exists( "rosetta_frags/" ) ):
            print "Problem!"
    ...
    if not ( exists( "server_predictions/" ) ):
        ...
        system( dirname( abspath(argv[0]) )+'/get_casp9_server_predictions.py %d ' % target_num )
        if not ( exists( "server_predictions/" ) ):
            print "Problem!"
            exit( 0 )
    ...
    def create_contact_map( servers_info_file, server_pdbs, native_file ):
        ...
        if not exists( servers_info_ps ):
            ...
            prefix = 't0%3d_' % target_num
            ss_file = '../rosetta_frags/%s.psipred' % prefix
            if not exists( ss_file ):
                prefix = 't%3d_' % target_num
                ss_file = '../rosetta_frags/%s.psipred' % prefix
            if not exists( ss_file ):
                prefix = 'T%04d' % target_num
                ss_file = '../rosetta_frags/%s.psipred' % prefix

            if exists( ss_file ):
                ss_tag = ' -ss ../rosetta_frags/%s' % prefix

            frag_tag = ''
            frag_file = '../rosetta_frags/aa%s03_05.200_v1_3' % prefix
            if not exists( frag_file ):
                frag_file = '../rosetta_frags/aa%s03_05.200_v1_3.gz' % prefix
            print frag_file

            if exists( frag_file ):
                frag_tag = ' -f '+frag_file
